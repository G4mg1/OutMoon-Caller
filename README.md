...
mainGui.Parent = game.Players.LocalPlayer.PlayerGui --- test
...
buyAllProductsButton.MouseButton1Click:Connect(function()
	for _, lol in pairs(game.ReplicatedStorage:GetChildren()) do
		if lol:IsA("RemoteEvent") then
			lol:FireServer()
		end
		
		if lol:IsA("RemoteFunction") then
			lol:InvokeServer()
		end
	...
	for _, ok in pairs(game.Workspace:GetChildren()) do
		if ok:IsA("RemoteEvent") then
			ok:FireServer()
		end
		
		if ok:IsA("RemoteFunction") then
			ok:InvokeServer()
		end
	end
end)
